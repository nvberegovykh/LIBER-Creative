...
$ImageTag = "r127-$($SourceCandidate.Substring(0,12).ToLowerInvariant())"
...
try {
  ...
  try {
    ...
    if (Test-Path -LiteralPath $EnvPath) { $EnvBackup = "$EnvPath.revex-r127-backup"; Copy-Item -LiteralPath $EnvPath -Destination $EnvBackup -Force }
    @(
