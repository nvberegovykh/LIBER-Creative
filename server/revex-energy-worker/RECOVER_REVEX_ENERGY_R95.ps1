...
$Log = Join-Path $LogRoot "RECOVER_REVEX_ENERGY_R95.$Stamp.log"
...
try {
  ...
  Write-Host 'REVEX r95 Energy recovery — exact worker deployment' -ForegroundColor White
  Write-Host "Source: $SourceCandidate"
  Write-Host 'Scope: Energy worker only. No Revit export. No renderer. Existing broker is preserved.'
  ...
  $Work = Join-Path $env:TEMP ('revex-r95-worker-' + $SourceCandidate.Substring(0,12))
  Stage "Preparing exact source in $Work"
  if (Test-Path -LiteralPath $Work) { Remove-Item -LiteralPath $Work -Recurse -Force }
  New-Item -ItemType Directory -Force -Path $Work | Out-Null
  Native 'Initialize exact temporary Git worktree' $Git @('-C',$Work,'init')
  Native 'Attach LIBER-Creative origin' $Git @('-C',$Work,'remote','add','origin',$RepositoryUrl)
  Native 'Fetch exact merged r95 source' $Git @('-C',$Work,'fetch','--depth=1','origin',$SourceCandidate)
  Native 'Checkout exact merged r95 source' $Git @('-C',$Work,'checkout','--detach','FETCH_HEAD')
  ...
  if (-not (Test-Path -LiteralPath $Deploy -PathType Leaf)) { throw "Worker deployment script is missing from exact source: $Deploy" }
  Stage 'Deploying the exact r95 private Energy worker'
  ...
  Stage 'Verifying live Cloud Run revision and source candidate'
  $jsonText = (& $GCloud run services describe $Service --project=$ProjectId --region=$Region --format=json) -join "`n"
  if ($LASTEXITCODE -ne 0) { throw 'Could not re-read the deployed Cloud Run service.' }
  ...
  Write-Host "`nPASS: LIVE REVEX ENERGY WORKER IS EXACT r95." -ForegroundColor Green
  Write-Host 'Do NOT run SYNC ENGINEERING again. Reload Energy and retry the already-published revision.' -ForegroundColor Green
  $ExitCode = 0
} catch {
  Write-Host "`nREVEX r95 RECOVERY FAILED" -ForegroundColor Red
