...
const DEFAULT_SCOPES = Object.freeze([...ALLOWED_SCOPES]);
...
const TOOL_DEFS = Object.freeze([
  { name:'observer_bootstrap', description:'Return the current LIBER/REVEX Observer guide, lease, scopes, project binding, and active bridge status.', inputSchema:{type:'object',properties:{},additionalProperties:false} },
  ...
  { name:'observer_create_focus', description:'Create one local bounded Observer focus in the active authorized REVEX browser session. This does not mutate Revit or Firestore project state.', scope:'observer.focus', method:'createFocus', inputSchema:{type:'object',properties:{spec:{type:'object'}},additionalProperties:false} },
...
function requestedScopes(value){
  const requested=Array.isArray(value)?value.map(clean).filter(Boolean):DEFAULT_SCOPES;
  const unknown=requested.filter(scope=>!ALLOWED_SCOPES.includes(scope));
  if(unknown.length) throw new HttpsError('invalid-argument',`Unsupported Observer scope: ${unknown.join(', ')}`);
  return [...new Set(requested.length?requested:DEFAULT_SCOPES)];
}
...
exports.listRevexObserverAgentProjects = onCall({region:REGION,timeoutSeconds:30}, async request=>{
  if(!request.auth?.uid) throw new HttpsError('unauthenticated','Sign in to LIBER Apps first.');
  const uid=String(request.auth.uid);
  const userSnap=await db.doc(`users/${uid}`).get();
  const user=userSnap.exists?userSnap.data()||{}:{};
  const admin=clean(user.role).toLowerCase()==='admin';
  const rows=new Map();
  const collect=snap=>snap.forEach(doc=>{ const data=doc.data()||{}; if(projectAccessRole(data,user,uid)) rows.set(doc.id,{projectId:doc.id,name:clean(data.name||data.title||data.code||doc.id),code:clean(data.code),role:projectAccessRole(data,user,uid)}); });
  if(admin){ collect(await db.collection('projects').limit(100).get()); }
  else {
    const [owned,member]=await Promise.all([
      db.collection('projects').where('ownerId','==',uid).limit(100).get(),
      db.collection('projects').where('memberIds','array-contains',uid).limit(100).get()
    ]);
    collect(owned); collect(member);
  }
  return {ok:true,projects:[...rows.values()].sort((a,b)=>a.name.localeCompare(b.name))};
});

exports.issueRevexObserverAgentClaim = onCall({region:REGION,timeoutSeconds:30}, async request=>{
  if(!request.auth?.uid) throw new HttpsError('unauthenticated','Sign in to LIBER Apps before issuing an AI session key.');
  const projectId=assertProjectId(request.data?.projectId);
  const focusId=assertFocusId(request.data?.focusId);
  const scopes=requestedScopes(request.data?.scopes);
  const uid=String(request.auth.uid);
  const access=await accessFor(projectId,uid);
  const claimKey=`liber_claim_${b64url(24)}`;
  const claimHash=sha256(claimKey);
  const createdAtMs=nowMs();
  const expiresAtMs=createdAtMs+CLAIM_TTL_MS;
  await db.doc(`revexObserverAgentClaims/${claimHash}`).set({
    schema:'liber.revex.observer-agent-claim.v1',projectId,focusId:focusId||null,scopes,ownerUid:uid,accessRole:access.role,
    createdAtMs,expiresAtMs,createdAt:FieldValue.serverTimestamp(),claimedAt:null,claimed:false
  },{merge:false});
  return {
    ok:true,schema:'liber.revex.observer-agent-counter.v1',claimKey,projectId,focusId:focusId||null,scopes,
    claimExpiresAt:iso(expiresAtMs),claimUrl:CLAIM_URL,mcpUrl:MCP_URL,guideUrl:GUIDE_URL,discoveryUrl:DISCOVERY_URL,counterUrl:COUNTER_URL,
    note:'Give this one-time claim key to exactly one AI session. The raw key is not stored server-side and expires after first claim or 10 minutes.'
  };
});
...
exports.claimRevexObserverAgentSession = onRequest({region:REGION,timeoutSeconds:30}, async (req,res)=>{
  ...
  try{
    ...
    const accessToken=`liber_obs_${b64url(32)}`;
    const tokenHash=sha256(accessToken);
    const sessionId=`obs_${b64url(12)}`;
    const claimedAtMs=nowMs();
    const expiresAtMs=claimedAtMs+SESSION_TTL_MS;
    let claim=null;
    await db.runTransaction(async tx=>{
      const snap=await tx.get(claimRef);
      if(!snap.exists) throw new Error('unknown_claim');
      claim=snap.data()||{};
      ...
      tx.set(db.doc(`revexObserverAgentSessions/${tokenHash}`),{
        schema:'liber.revex.observer-agent-session.v1',sessionId,projectId:claim.projectId,focusId:claim.focusId||null,
        scopes:Array.isArray(claim.scopes)?claim.scopes:DEFAULT_SCOPES,ownerUid:claim.ownerUid,accessRole:claim.accessRole||null,
        issuedAtMs:claimedAtMs,expiresAtMs,issuedAt:FieldValue.serverTimestamp(),revoked:false,lastUsedAtMs:claimedAtMs
      },{merge:false});
    });
    return json(res,200,{
      ok:true,schema:'liber.revex.observer-agent-session.v1',sessionId,accessToken,projectId:claim.projectId,focusId:claim.focusId||null,
      scopes:Array.isArray(claim.scopes)?claim.scopes:DEFAULT_SCOPES,expiresAt:iso(expiresAtMs),mcpUrl:MCP_URL,guideUrl:GUIDE_URL,discoveryUrl:DISCOVERY_URL
    });
  }catch(error){
    const reason=errorText(error);
    const code=reason.includes('already_used')?409:reason.includes('expired')?410:reason.includes('unknown_claim')?404:500;
...
exports.pullRevexObserverAgentRequests = onCall({region:REGION,timeoutSeconds:30}, async request=>{
  ...
  const bridge=request.data?.bridge||{};
  const clientId=clean(bridge.clientId).slice(0,160)||`revex_${uid}`;
...
exports.completeRevexObserverAgentRequest = onCall({region:REGION,timeoutSeconds:30}, async request=>{
  ...
  const ref=db.doc(`projects/${projectId}/revexObserverAgentRequests/${requestId}`);
  const success=request.data?.success===true;
  let result=request.data?.result??null;
...
async function dispatchToRevex(session,tool,args){
  const requestId=`req_${b64url(12)}`;
  const ref=db.doc(`projects/${session.projectId}/revexObserverAgentRequests/${requestId}`);
  const createdAtMs=nowMs();
  await ref.set({
  ...
  throw new Error('No active authorized REVEX session is servicing this project. Open REVEX on the selected project and try again.');
}

async function bootstrap(session){
  const bridgeSnap=await db.doc(`projects/${session.projectId}/revexObserverAgentBridge/current`).get();
  const bridge=bridgeSnap.exists?bridgeSnap.data()||{}:null;
  const age=bridge?nowMs()-Number(bridge.lastSeenAtMs||0):Infinity;
  return {
    schema:'liber.revex.observer-agent-bootstrap.v1',sessionId:session.sessionId,projectId:session.projectId,focusId:session.focusId||null,
    scopes:session.scopes||[],expiresAt:iso(Number(session.expiresAtMs)),observerMethod:'snapshot -> invariants -> preview -> bounded owner -> observe again -> compare delta -> record step',
    discoveryUrl:DISCOVERY_URL,guideUrl:GUIDE_URL,counterUrl:COUNTER_URL,mcpUrl:MCP_URL,
    bridge:{online:age<7000,lastSeenAt:bridge?.lastSeenAtMs?iso(Number(bridge.lastSeenAtMs)):null,observerVersion:bridge?.observerVersion||null,revision:bridge?.revision||null,activeView:bridge?.activeView||null},
    mutationAvailable:false,note:'This lease exposes the read-only REVEX Observer and local focus journal only. Revit remains model authority.'
  };
}
...
exports.revexObserverMcp = onRequest({region:REGION,timeoutSeconds:60,memory:'512MiB'}, async (req,res)=>{
  cors(res); if(req.method==='OPTIONS') return res.status(204).send('');
  if(req.method!=='POST') return json(res,405,{error:'method_not_allowed'});
  const loaded=await loadSessionFromRequest(req);
  if(loaded.error) return json(res,401,{error:loaded.error});
  const {ref:sessionRef,session}=loaded;
  ...
  try{
    ...
    if(rpc.method==='initialize') return json(res,200,{jsonrpc:'2.0',id,result:{protocolVersion:clean(rpc.params?.protocolVersion)||'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'LIBER REVEX Observer',version:'20260917r151-agent-gate1'}}});
    ...
    const name=clean(rpc.params?.name); const args=rpc.params?.arguments||{};
    const tool=TOOL_DEFS.find(row=>row.name===name);
