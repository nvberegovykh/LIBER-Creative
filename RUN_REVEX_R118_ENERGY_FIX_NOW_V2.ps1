...
$EnergySource = "8bbc4df21bbce402d617bfa9950fc57c0c6c5dc5"
$Revision = "eng_20260817T032812010Z"
$Repo = "https://github.com/nvberegovykh/LIBER-Creative.git"
$Work = Join-Path $env:TEMP ("REVEX-R118-V2-" + [guid]::NewGuid().ToString("N"))
...
try {
  Write-Host "REVEX r118 Energy recovery v2" -ForegroundColor Cyan
  Write-Host "Exact source: $EnergySource"
  Write-Host "Preserved revision: $Revision"
  Write-Host "Scope: Energy worker + authenticated broker only. No Revit sync. No BIM/Docs/Render mutation." -ForegroundColor Green
  ...
  if ((Invoke-Native $Git @("-C", $Source, "fetch", "--depth", "1", "origin", $EnergySource)) -ne 0) { throw "Exact Energy source fetch failed." }
  if ((Invoke-Native $Git @("-C", $Source, "checkout", "--detach", "FETCH_HEAD") -Quiet) -ne 0) { throw "Exact Energy source checkout failed." }
  ...
  if (-not (Test-Path -LiteralPath $deploy -PathType Leaf)) { throw "argv-safe Energy deployer is missing." }
  ...
  $code = Invoke-Native "powershell.exe" @(
    ...
    "-ProjectId", $ProjectId,
    "-Region", $Region,
    "-SourceCandidate", $EnergySource,
    "-NoPause"
  )
  if ($code -ne 0) { throw "Fixed Energy deployment failed with exit code $code." }

  Write-Host "PASS: fixed r118 Energy edge deployed." -ForegroundColor Green
  ...
  Write-Host "REVEX r118 Energy recovery v2 stopped safely: $($_.Exception.Message)" -ForegroundColor Red
  $ExitCode = 1
}
finally {
