...
must(ui,"wallt-ui-r138.js?v=20260820r147-release1",'visible context-aware WALLT loader');
must(ui,"mobile-sheet-r142.js?v=20260820r147-release1",'reused-node mobile sheet loader');
...
must(walltUi,"const BUILD='20260820r144-wallt-context1'",'visible WALLT build');
...
must(blocks,"button.hidden=false;button.disabled=!hosted()",'Blocks family action is discoverable with an explicit browser-only state');
must(blocks,'data-family-insert','Blocks explicit insert confirmation');
mustNot(blocks,'setInterval(','Blocks lifecycle must be event-driven');
...
must(spaceShield,'normalized == "space must have a height greater than 0"','exact native zero-height Space failure match');
must(spaceShield,'normalized == "space must have a height greater than zero"','localized numeric-word compatibility match');
