...
if source.count('reconcile_publication_message_severity(messages, publication_threshold_met)') < 2:
    raise SystemExit('publication reconciliation helper is not both defined and invoked')
...
elif 'accepted gbXML path does not invoke publication severity reconciliation' not in verify:
