...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="revex-r49-live-comcheck-") as temp:
        ...
        from comcheck_backstop import ComcheckClient, run_official_backstop, BACKSTOP_CODE

        # The DWR interface stub is generated by the exact live COMcheck server from
        # its Java method signature. Inspect it on every live QA run rather than
        # guessing the upload parameter count/order. DWR stores the generated proxy
        # on a local object (currently `p`), then registers that object as
        # ProjectService, so accept either form instead of assuming the variable name.
        inspection = ComcheckClient(
            "https://legacy-comcheck.energycode.pnl.gov/CheckWeb/",
            lambda status, **detail: log.write("COMCHECK_INTERFACE_INSPECTION", status, **detail),
            timeout_seconds=30,
        )
        inspection.start()
        stub = inspection._request("GET", "dwr/interface/ProjectService.js").decode("utf-8", errors="replace")
        match = re.search(r"(?:ProjectService|p)\.uploadProject\s*=\s*function\s*\(([^)]*)\)\s*\{(.*?)\n\}", stub, re.S)
        if not match:
            raise RuntimeError("Official COMcheck ProjectService.js exposes no uploadProject stub")
        signature = ",".join(part.strip() for part in match.group(1).split(","))
        body = re.sub(r"\s+", " ", match.group(2)).strip()
        if len(body) > 900:
            body = body[:900] + "..."
        print(f"REVEX_COMCHECK_UPLOAD_SIGNATURE={signature}")
        print(f"REVEX_COMCHECK_UPLOAD_STUB={body}")
