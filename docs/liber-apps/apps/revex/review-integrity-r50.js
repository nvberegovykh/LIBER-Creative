...
const BUILD = '20260813r49-review1';
...
if (!window.__revexReviewIntegrityR50) {
  ...
  function patchViewer(v) {
    ...
    v.__reviewSection = {
      enabled: Boolean(v.section?.enabled),
      minX: 0, maxX: 1,
      minY: 0, maxY: 1,
      minZ: 0, maxZ: 1
    };
    ...
    v.setSectionDimension = function setSectionDimension(dimension, requested) {
      ...
      const s = this.__reviewSection;
      const baseSize = this.bounds.getSize(new THREE.Vector3());
      const value = Math.max(0.01, Number(requested) || 0.01);
      const config = dimension === 'width'
        ? ['minX', 'maxX', baseSize.x]
        : dimension === 'length'
          ? ['minZ', 'maxZ', baseSize.z]
          : ['minY', 'maxY', baseSize.y];
      const [minKey, maxKey, full] = config;
      const span = clamp(value / Math.max(full, 0.0001), 0.002, 1);
    ...
    syncSectionInputs(v);
    diagnostic('INFO', 'VIEWER_REVIEW_CONTRACT', 'Six-face visible section box and review controls installed.');
    return true;
  }
  ...
  function handleViewerClick(event) {
    ...
    if (!button) return;
    const id = button.id;
    if (!['fit-model', 'fit-model-rail', 'detail-toggle', 'walk-toggle', 'section-toggle', 'section-reset', 'areas-toggle'].includes(id)) return;
    ...
    if (id === 'areas-toggle') return toggleAreas(v, button);
    if (id === 'walk-toggle') {
  ...
  function ensureAreaButton() {
    const tools = $('.viewer-tools');
    if (!tools || $('#areas-toggle')) return;
    const button = document.createElement('button');
    button.id = 'areas-toggle';
    button.className = 'button ghost compact';
    button.type = 'button';
    button.textContent = 'Areas';
    button.title = 'Show Revit Room / Space / Area extents from the current revision metadata';
    const section = $('#section-toggle');
    if (section) section.insertAdjacentElement('afterend', button); else tools.appendChild(button);
    refreshAreaControl();
  }

  function areaRows(v) {
    return (v?.data?.elements || []).filter((row) => /^(areas?|rooms?|spaces?)$/i.test(String(row.category || '').trim()) && row.bbox?.min && row.bbox?.max);
  }

  function refreshAreaControl() {
    ensureAreaButton();
    const button = $('#areas-toggle');
    const v = activeViewer();
    if (!button || !v) return;
    const count = areaRows(v).length;
    button.hidden = count === 0;
    button.title = count ? `Show ${count} current Revit Room / Space / Area extents` : 'No Room / Space / Area extents in this revision';
  }

  function toggleAreas(v, button) {
    const on = !button.classList.contains('active');
    button.classList.toggle('active', on);
    if (!v.__reviewAreaHelpers) v.__reviewAreaHelpers = [];
    v.__reviewAreaHelpers.forEach((helper) => v.scene.remove(helper));
    v.__reviewAreaHelpers = [];
    if (on) {
      for (const row of areaRows(v).slice(0, 400)) {
        const box = v.box?.(row);
        if (!box || box.isEmpty()) continue;
        const helper = new THREE.Box3Helper(box, 0x5bb7ff);
        helper.name = `REVEX_AREA_${row.id || ''}`;
        v.scene.add(helper);
        v.__reviewAreaHelpers.push(helper);
      }
    }
    v.requestRender?.();
  }
  ...
  function renderHiddenRegistry() {
    ...
    registry.innerHTML = `<div class="tree-group">Hidden / deleted review records · ${overlays.length}</div>` + (overlays.length ? overlays.map((row) => {
      ...
      return `<div class="review-hidden-row" data-overlay-key="${esc(stableElementKey(row))}">
        <div><strong>${esc(row.category || source?.category || 'BIM element')}</strong><span>${esc(source?.name || source?.type || row.elementId || row.id || stableElementKey(row))}</span><small>${status}${source ? ' · present in current revision' : ' · not present in current Revit revision'}</small></div>
  ...
  function enforcePositionVersionOwnership() {
    // Chapter-level "Versions" was an old wiring mistake. Inspiration and renderings
    // remain chapter context; version history is rendered only inside the selected position.
  ...
  window.addEventListener('revex:viewer-mode', () => setTimeout(() => { ensureViewerPatched(); refreshAreaControl(); }, 20));

  const hiddenButton = $('#show-hidden-elements');
  hiddenButton?.addEventListener('click', () => setTimeout(renderHiddenRegistry, 0));
