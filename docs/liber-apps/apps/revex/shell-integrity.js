(function(root){
  ...
  const VERSION='0.8.0';
  if(root.__revexShellR19) return;
  root.__revexShellR19=true;
  ...
  function project(){
    ...
    return {
      ...
      name:String(select?.selectedOptions?.[0]?.textContent||'REVEX project').trim()
    };
  }
  ...
  async function copyInvitation(){
    ...
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
      emailInput?.focus?.();
      return notify('Enter the recipient email so REVEX can grant the correct project access.','error');
    }
  ...
  function bindInviteCopy(){
    ...
    button.id='invite-copy'; button.type='button';
  ...
  function postProject(reason='selection'){
    try{
      ...
      root.chrome.webview.postMessage({
        type:'liber:revex-project-selected',projectId:id,
        specProjectId:String(qs.get('specProjectId')||'').trim()||null,
        projectName:name,source:'companion-immediate',reason,build:BUILD
      });
      return true;
  ...
  function bindProjectBridge(){
    ...
    if(!select.dataset.revexNativeProjectBridge){
      select.dataset.revexNativeProjectBridge='r19';
      select.addEventListener('change',()=>postProject('change'));
      select.addEventListener('input',()=>postProject('input'));
      new MutationObserver(()=>postProject('options')).observe(select,{childList:true});
    }
    postProject('bind');
  }
  ...
  function announceReady(reason){
    try{
      ...
      if(!input||!root.chrome?.webview?.postMessage) return false;
      input.dataset.liberRevexSyncHandlerReady='1';
      root.__liberRevexNativeSyncReady=true;
      root.chrome.webview.postMessage({type:'liber:revex-native-sync-ready',build:BUILD,projectId:project().id||null,reason});
      return true;
    }catch(_){return false;}
  }
  ...
  function alignParentShell(){
    try{
      ...
      const manager=parent?.appsManager;
      if(!manager) return;
      if(!manager.__revexR19KeepAlive&&typeof manager.isKeepAliveApp==='function'){
        const original=manager.isKeepAliveApp.bind(manager);
        manager.isKeepAliveApp=(src)=>/apps\/revex\/index\.html/i.test(String(src||''))?false:original(src);
        manager.__revexR19KeepAlive=true;
      }
      if(!manager.__revexR19FreshLaunch&&typeof manager.openAppInShell==='function'){
        const original=manager.openAppInShell.bind(manager);
        manager.openAppInShell=(app,appUrl)=>{
          const isRevex=String(app?.id||'')==='revex'||/apps\/revex\/index\.html/i.test(String(appUrl||''));
          if(!isRevex) return original(app,appUrl);
          let url=String(appUrl||'');
          try{
            const u=new URL(url,parent.location.href);
            u.searchParams.set('build',BUILD);u.searchParams.set('fresh',String(Date.now()));url=u.href;
          }catch(_){url+=(url.includes('?')?'&':'?')+'build='+BUILD+'&fresh='+Date.now();}
          return original(app,url);
        };
        manager.__revexR19FreshLaunch=true;
      }
      if(Array.isArray(manager.apps)){
        const app=manager.apps.find(row=>row?.id==='revex');
        if(app){app.version=VERSION;app.lastUpdated='2026-08-10';app.path='apps/revex/index.html?build='+BUILD;}
      }
      const card=parent.document?.querySelector?.('.app-card[data-app-id="revex"] .app-version');
      if(card) card.textContent='v'+VERSION;
    }catch(error){console.warn('[REVEX] shell alignment',error);}
  }

  function bind(){
    bindInviteCopy();bindProjectBridge();alignParentShell();
  }
  ...
  root.addEventListener('load',()=>{
    bind();
    announceReady('window-load');
    setTimeout(()=>announceReady('window-load-settle'),250);
  },{once:true});
