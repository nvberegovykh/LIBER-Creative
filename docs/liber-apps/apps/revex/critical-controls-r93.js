(function(root){
  'use strict';
  const BUILD='20260816r93-critical-controls1';
  ...
  function publishLocal(rows){
    ...
    root.dispatchEvent(new CustomEvent('revex:bim-overlays-changed',{detail:{projectId:s.projectId,overlays:rows||[],source:'critical-controls-r93'}}));
  }
  async function restoreAll(){
    ...
    if(!targets.length){publishLocal(all);if(button){button.disabled=true;button.textContent='All elements visible';}return {restored:0};}
  ...
  async function refreshRestore(){
    ...
    try{const rows=(await store.listBimOverlays?.(projectId))||[];publishLocal(rows);const count=rows.filter(hidden).length;button.disabled=!count;button.textContent=count?`Restore all hidden / deleted · ${count}`:'All elements visible';}
  ...
  function install(){
    ...
    if(button&&!button.dataset.revexR93Canonical){
      ...
      button.addEventListener('click',event=>{
        ...
        void restoreAll().catch(error=>{if(button){button.disabled=false;button.textContent='Restore all hidden / deleted';}diag('ERROR','BIM_RESTORE_ALL_CANONICAL',error?.message||String(error));const node=document.getElementById('viewer-message');if(node){node.textContent=`Restore All failed: ${error?.message||error}`;node.hidden=false;}},true);
      },true);
    }
    void refreshRestore();
  }
  ...
  root.addEventListener('revex:energy-open',()=>setTimeout(install,0));
