(function(root){
  ...
  async function upload(projectId,revision,file){const path=`projects/${projectId}/library/revex/revisions/${revision}/printing-sets/${safe(file.name)}`;const ref=Store.api.ref(Store.fs.storage,path);post('docs-upload-start',{path,bytes:file.size});await Store.api.uploadBytes(ref,file,clone({contentType:'application/pdf'}));post('docs-upload-complete',{path,bytes:file.size});return{path,url:await Store.api.getDownloadURL(ref)};}
  function localDocs(manifest,files){return(manifest?.sets||[]).map(set=>{const file=byName(files,set.fileName);return file?{name:file.name,size:file.size,url:URL.createObjectURL(file),set}:null;}).filter(Boolean);}
  ...
  Store.syncPackage=async function(fileList,preferredProjectId,preferredSpecProjectId){
    ...
    result.printingSets=manifest;result.printingDocs=localDocs(manifest,files);
    const announce=()=>{try{root.dispatchEvent(new CustomEvent('revex:r20-revision',{detail:{projectId:result.projectId,revision:result.revision,cloud:!!result.cloud}}));}catch(_){}};
    if(!manifest?.sets?.length){announce();return result;}
    if(!result.cloud||!Store.isCloud()||!Store.user?.uid||!Store.fs?.storage){post('docs-local-preview',{sets:manifest.sets.length});announce();return result;}
    const records=[];
    ...
    result.printingDocs=records;post('docs-index-complete',{sets:records.length,pages:records.reduce((n,r)=>n+(r.sheetIndex?.length||0),0)});announce();return result;
  };
