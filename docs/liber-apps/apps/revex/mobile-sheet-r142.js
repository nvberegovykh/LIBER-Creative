...
function config(view){if(view==='bim')return {a:{label:'Model',node:document.querySelector('#view-bim>.rail'),key:'bim-rail'},b:{label:'Properties',node:byId('bim-inspector'),key:'bim-inspector'}};if(view==='design')return {a:{label:'Selector',node:document.querySelector('#view-design>.chapter-rail'),key:'design-rail'},b:{label:'Position',node:byId('design-inspector'),key:'design-inspector'}};return null;}
function moveConfigured(view){const c=config(view),s=sheet(),body=byId('revex-r142-sheet-body');if(!c||!body){s.hidden=true;return;}for(const p of Object.values(c)){if(!p.node)continue;remember(p.node,p.key);}byId('revex-r142-sheet-a').textContent=c.a.label;byId('revex-r142-sheet-b').textContent=c.b.label;s.hidden=false;currentView=view;if(currentPane)openPane(currentPane);}
...
function restoreAll(){closePane();for(const k of['bim-rail','bim-inspector','design-rail','design-inspector'])restore(k);sheet().hidden=true;currentView='';}
function activeView(){const b=document.querySelector('.main-nav [data-view].active');return String(b?.dataset?.view||'');}
function sync(){if(!mobile()){restoreAll();return;}const v=activeView();if(v==='bim'||v==='design')moveConfigured(v);else{for(const k of['bim-rail','bim-inspector','design-rail','design-inspector'])restore(k);sheet().hidden=true;currentView='';currentPane='';}}
