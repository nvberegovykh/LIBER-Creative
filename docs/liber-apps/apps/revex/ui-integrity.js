(function(root){
  ...
  function loadScript(src,key,type='text/javascript'){if(document.querySelector(`script[data-revex-runtime="${key}"]`))return;const script=document.createElement('script');script.dataset.revexRuntime=key;script.src=src;script.type=type;script.async=false;script.onerror=()=>root.__revexBrowserDiagnostics?.emit?.('ERROR','R75_RUNTIME',`Could not load ${src}.`,{initiator:'ui integrity loader'});document.head.appendChild(script);}
  ...
  function loadCurrentRepairs(){
    // r75 deliberately does NOT load docs-pages-r68, appearance-r70,
    // companion-state-r71, model-filter-r71, viewer-runtime-r72 or material-modal-r72.
    // Those overlapping runtimes caused duplicate hydration/filter ownership,
    // timer polling and main-thread PDF splitting. Their persistent data schemas remain compatible.
  ...
  console.log('[REVEX] UI integrity '+BUILD,{projectId:'visible',reviewRuntime:'r49-review2',docsPages:'native-pdf-page-navigation-no-main-thread-split',viewerPolish:'r68',appearance:'single-owner-r75',modelFilter:'single-owner-r75',viewerRuntime:'r75-preemptible-proxy-first',materialIntegration:'embedded-properties-architextures-user-download-auto-apply',restoreAll:'batched',targetFps:30,spatialObjects:'invisible'});
})(window);
