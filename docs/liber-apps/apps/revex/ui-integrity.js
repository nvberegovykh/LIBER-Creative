(function(root){
  ...
  function installCompanion(){
    ...
    try{
      if(!document.querySelector('link[rel="manifest"][data-revex-manifest]')){
        const link=document.createElement('link');link.rel='manifest';link.href='manifest.webmanifest?v='+BUILD;link.dataset.revexManifest='1';document.head.appendChild(link);
      }
      if(!document.querySelector('meta[name="theme-color"][data-revex-theme]')){
        const meta=document.createElement('meta');meta.name='theme-color';meta.content='#080a0e';meta.dataset.revexTheme='1';document.head.appendChild(meta);
      }
      ...
      if(!document.querySelector('meta[name="apple-mobile-web-app-title"]')){
        const meta=document.createElement('meta');meta.name='apple-mobile-web-app-title';meta.content='REVEX';document.head.appendChild(meta);
      }
      if(!document.querySelector('link[rel="apple-touch-icon"]')){
        const link=document.createElement('link');link.rel='apple-touch-icon';link.href='/liber-apps/images/LIBER%20LOGO.png';document.head.appendChild(link);
      }
    }catch(_){}

    if('serviceWorker' in navigator&&location.protocol==='https:') navigator.serviceWorker.register('/liber-apps/sw.js').catch(()=>{});

    if(!document.getElementById('revex-install-banner-style')){
      const style=document.createElement('style');style.id='revex-install-banner-style';style.textContent=`
        .revex-install-banner{position:fixed;inset:0 0 auto 0;z-index:10000;min-height:46px;box-sizing:border-box;display:flex;align-items:center;justify-content:center;gap:12px;padding:8px max(12px,env(safe-area-inset-right)) 8px max(12px,env(safe-area-inset-left));background:rgba(8,10,14,.97);border-bottom:1px solid rgba(255,255,255,.14);box-shadow:0 8px 24px rgba(0,0,0,.28);backdrop-filter:blur(16px);color:#eef2f7;font-size:12px}.revex-install-banner[hidden]{display:none!important}.revex-install-copy{display:flex;align-items:baseline;gap:8px;min-width:0}.revex-install-copy strong{white-space:nowrap;font-size:12px}.revex-install-copy span{opacity:.72;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.revex-install-actions{display:flex;align-items:center;gap:6px;flex:0 0 auto}.revex-install-action,.revex-install-close{border:1px solid rgba(255,255,255,.18);border-radius:7px;background:rgba(255,255,255,.08);color:inherit;min-height:30px;padding:5px 10px;font:inherit;cursor:pointer}.revex-install-action{background:#eef2f7;color:#0b0d11;font-weight:650}.revex-install-close{width:30px;padding:0;font-size:18px;line-height:1}body.revex-install-banner-open .app-shell{padding-top:47px;box-sizing:border-box}@media(max-width:720px){.revex-install-banner{justify-content:space-between;gap:8px;min-height:52px}.revex-install-copy{display:block;line-height:1.25}.revex-install-copy span{display:block;max-width:52vw;margin-top:2px}body.revex-install-banner-open .app-shell{padding-top:53px}}`;
      document.head.appendChild(style);
    }

    let banner=document.getElementById('revex-install-banner');
    if(!banner){
      banner=document.createElement('aside');banner.id='revex-install-banner';banner.className='revex-install-banner';banner.hidden=true;banner.setAttribute('role','region');banner.setAttribute('aria-label','Install REVEX Companion');
      banner.innerHTML='<div class="revex-install-copy"><strong>Install REVEX Companion</strong><span id="revex-install-hint">Open REVEX like an app from your desktop or home screen.</span></div><div class="revex-install-actions"><button id="revex-install-action" class="revex-install-action" type="button">Install app</button><button id="revex-install-close" class="revex-install-close" type="button" aria-label="Close install banner">×</button></div>';
    ...
    root.matchMedia?.('(display-mode: standalone)').addEventListener?.('change',()=>{if(isStandalone())hide();});
    show();
  }
  ...
  console.log('[REVEX] UI integrity '+BUILD,{projectId:'visible',labels:'source-text',observer:false,pwaInstallBanner:true,revitSuppressed:true});
})(window);