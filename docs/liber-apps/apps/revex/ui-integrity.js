(function(root){
  ...
  function loadScript(src,key,type='text/javascript'){if(document.querySelector(`script[data-revex-runtime="${key}"]`))return;const script=document.createElement('script');script.dataset.revexRuntime=key;script.src=src;script.type=type;script.async=false;script.onerror=()=>root.__revexBrowserDiagnostics?.emit?.('ERROR','R80_RUNTIME',`Could not load ${src}.`,{initiator:'ui integrity loader'});document.head.appendChild(script);}
  ...
  function loadCurrentRepairs(){
    // r75 remains the consolidated data/UI owner. r79 owns the proven live BIM
    // interface repairs. r80 makes Energy failure display revision-scoped so an
    // old worker failure can never masquerade as the current Engineering state.
  ...
  console.log('[REVEX] UI integrity '+BUILD,{projectId:'visible',reviewRuntime:'r49-review2',docsPages:'native-pdf-page-navigation-no-main-thread-split',viewerPolish:'r68',appearance:'single-owner-r75+auth-rehydrate-r79',modelFilter:'single-owner-r75',viewerRuntime:'r75-preemptible+r79-adaptive-exact',materialIntegration:'embedded-properties-architextures-user-download-auto-apply',restoreAll:'setDoc-fallback-r79',selection:'viewer-to-properties-r79',energyDiagnostics:'current-revision-only-r80',targetFps:30,spatialObjects:'invisible'});
})(window);
