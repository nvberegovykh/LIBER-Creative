 ...
 * Reuses the same secure OpenAI proxy/configuration as the legacy assistant widget,
 * but exposes a small programmatic API for REVEX, Revit and future LIBER apps.
 */
(function (root) {
  'use strict';
  ...
  class WalltAgent {
    ...
    legacyIntegration() {
      ...
      for (const w of candidates) {
        const agent = w.chatgptIntegration;
        if (agent?.openaiFetch || agent?.callWALLE) return agent;
      }
      return null;
    }
    ...
    async init() {
      ...
      this._initPromise = (async () => {
        ...
        if (!this.proxyUrl && !this.apiKey) {
          this.proxyUrl = `https://${DEFAULT_REGION}-${DEFAULT_PROJECT}.cloudfunctions.net/openaiProxy`;
        }
        this.ready = true;
        return this;
      })();
    ...
    async openaiFetch(path, init = {}) {
      await this.init();
      const legacy = this.legacyIntegration();
      if (legacy?.openaiFetch) {
        return legacy.openaiFetch(path, init);
      }
      const base = this.proxyUrl || 'https://api.openai.com';
    ...
    async response({ instructions, input, model = '', timeoutMs = 60000 }) {
      ...
      try {
        const response = await this.openaiFetch('/v1/responses', {
          method: 'POST',
          body: JSON.stringify({
            model: model || this.model || DEFAULT_MODEL,
        ...
        if (!response.ok) throw new Error(json?.error?.message || json?.message || `WALLT request failed (${response.status})`);
        const text = outputText(json);
        if (!text) throw new Error('WALLT returned no visible output.');
        return text;
      } finally {
        clearTimeout(timer);
      }
    ...
    async ask(message, options = {}) {
      ...
      const instructions = options.instructions || [
        'You are WALLT, the LIBER assistant.',
        'Be concise, technically precise and action-oriented.',
        'Respect user ownership of project data. LIBER and REVEX are tools, not owners of user data.',
        `Current app context: ${JSON.stringify(context)}`
      ].join('\n');
      return this.response({ instructions, input, model: options.model || '' });
    }
    ...
    async planRender({ message, context = {}, history = [] }) {
      ...
      const text = await this.response({ instructions, input });
      const plan = parseJsonLoose(text);
      if (plan) return plan;
      return {
        assistant: text,
        intent: 'none',
        rendair: { action: 'none', prompt: '', environment: 'Natural daylight', staging: 'Preserve modeled objects only', people: 'None' },
        revit: { capture: false, sync: false },
        save: { designBook: false, chapterId: null },
        needsUser: false
      };
    }
  }

  root.WalltAgent = WalltAgent;
  root.walltAgent = root.walltAgent || new WalltAgent();

  // New canonical name; retain old aliases only so existing LIBER code keeps working.
  try {
    const legacy = root.chatgptIntegration;
    if (legacy) root.wallt = legacy;
  } catch (_) {}
})(window);
