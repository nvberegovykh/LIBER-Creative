...
try {
  ...
  Write-Host "Energy source: $EnergySource"
  Write-Host "Preserved revision: $Revision"
  Write-Host "No Revit sync, no Dynamo, no add-in replacement, no BIM/Docs/Render mutation." -ForegroundColor DarkGray
  Write-Host ""
  ...
  if ((Invoke-Native $Git @("-C",$Source,"checkout","--detach","FETCH_HEAD") -Quiet) -ne 0) { throw "Exact r118 checkout failed." }
  ...
  Write-Host ""
  Write-Host "PASS: r118 Energy edge deployed." -ForegroundColor Green
  ...
  Write-Host ""
  Write-Host "REVEX r118 Energy recovery stopped safely." -ForegroundColor Red
  ...
  Write-Host ""
  if ($ExitCode -eq 0) {
    Write-Host "NEXT: authorize/retry $Revision once in Energy." -ForegroundColor Cyan
  } else {
    Write-Host "Nothing in Revit or the immutable Engineering revision was replaced." -ForegroundColor Yellow
  }
